...
struct RandomSearchResultView: View {
    ...
    var body: some View {
        let candidates: [SongInfo?] = [MockSongs.song1, MockSongs.song2, MockSongs.song3, nil]
        let selected: SongInfo? = candidates.randomElement()!
        
        return NavigationStack(path: $path) {
            SearchResultView(songInfo: selected, path: $path)
        }
    }
}
